...
class QspiIoBFM(port: qspiIO, clock: Clock)
{
  ...
  def receiveFunctionCode() : Int =
  {
    ...
    val obtainedInstructionInt : Int = Integer.parseInt(obtainedInstructionStr, 2)
  ...
  def expectFunctionCode(funcCode: Int) = {
    ...
    assert(obtainedFunctCode == funcCode, s"Expected $funcCode, got $obtainedFunctCode")
  }
  ...
  def expectAddress(address: Int) = {
    ...
    assert(address == obtainedAddress, s"Expected $address, got $obtainedAddress")
  }
}
