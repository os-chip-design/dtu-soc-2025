   ...
   * | 1   | receiver has data        |
   *
   * Reading from address 0x00 will remove a character from the recieve buffer. If the
  ...
  class MemoryMappedUart(
    ...
    val io = IO(new Bundle {
      ...
      /* internal RTS and CTS pins for flow control || CTS == LOW, no reading, RTS == LOW, when transmitting*/
      //val RTS = Output(Bool())
      //val CTS = Input(Bool())
