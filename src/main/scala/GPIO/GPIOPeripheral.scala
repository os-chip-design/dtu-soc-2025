...
class GPIOPeripheral(addrWidth: Int, nofGPIO: Int) extends Module {
    ...
    //val gpio_input = RegNext(gpio_module.io.gpio_input, 0.U(nofGPIO.W))
    ...
    // val parallel_write   = Wire(UInt(addrWidth.W))
    // val serial_write     = Wire(UInt(1.W))
    ...
    switch (write_state_reg) {
        ...
        is (State.read_parallel_reg) {
            ...
            {
                gpio_output := writeReg //parallel_write
            }.otherwise {
                gpio_input  := VecInit(writeReg(7, 0).asBools) //parallel_write
            }
            wrAckReg        := true.B
